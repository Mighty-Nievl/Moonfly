document.addEventListener('DOMContentLoaded', () => {
  ...
  // Appearance Controls
  const ctrlCardRadius = document.getElementById('ctrl-card-radius');
  const ctrlCardOpacity = document.getElementById('ctrl-card-opacity');
  const ctrlCardBlur = document.getElementById('ctrl-card-blur');
  const ctrlBmRadius = document.getElementById('ctrl-bm-radius');
  const ctrlBmOpacity = document.getElementById('ctrl-bm-opacity');
  const ctrlBmBlur = document.getElementById('ctrl-bm-blur');
  ...
  document.addEventListener('click', (e) => {
      if (!mainMenu.contains(e.target) && e.target !== menuBtn && e.target !== floatingMenuBtn) {
  ...
  let showClock = true;
  let showDate = true;
  let showHeader = true; // New: Header visibility
  let isDarkMode = true; // New: Dark Mode
  ...
  function loadSettings() {
      const keys = ['isLocked', 'showTitles', 'showClock', 'showDate', 'showHeader', 'isDarkMode', 'accentColor', 'viewMode', 'columnCount', 'titleAlign',
                    'cardRadius', 'cardOpacity', 'cardBlur', 'bmRadius', 'bmOpacity', 'bmBlur'];
      chrome.storage.sync.get(keys, (res) => {
          ...
          if (res.showHeader !== undefined) showHeader = res.showHeader;
          if (res.isDarkMode !== undefined) isDarkMode = res.isDarkMode;
          ...
          if (res.cardRadius !== undefined) cardRadius = res.cardRadius;
          if (res.cardOpacity !== undefined) cardOpacity = res.cardOpacity;
          if (res.cardBlur !== undefined) cardBlur = res.cardBlur;
          if (res.bmRadius !== undefined) bmRadius = res.bmRadius;
          if (res.bmOpacity !== undefined) bmOpacity = res.bmOpacity;
          if (res.bmBlur !== undefined) bmBlur = res.bmBlur;
          ...
          updateDateState();
          updateHeaderState();
          updateDarkMode(); // New: Apply Dark Mode
          updateAccentColor();
          updateViewModeUI();
          updateColumnCountUI();
          updateAlignUI();
          updateAppearance(); // New: Apply appearance settings
  ...
  function updateHeaderState() {
      ...
      if (showHeader) {
          ...
          if(floatingMenuBtn) floatingMenuBtn.classList.add('hidden');
      } else {
          if(headerToggle) headerToggle.classList.remove('active');
          document.body.classList.add('header-hidden');
          if(floatingMenuBtn) floatingMenuBtn.classList.remove('hidden');
      }
  }
  ...
  function updateAppearance() {
    ...
    root.style.setProperty('--card-opacity', cardOpacity / 100);
    root.style.setProperty('--card-blur', `${cardBlur}px`);
    
    root.style.setProperty('--bookmark-radius', `${bmRadius}px`);
    root.style.setProperty('--bookmark-opacity', bmOpacity / 100);
    root.style.setProperty('--bookmark-blur', `${bmBlur}px`);
    
    // Sync Inputs
    if(ctrlCardRadius) ctrlCardRadius.value = cardRadius;
    if(ctrlCardOpacity) ctrlCardOpacity.value = cardOpacity;
    if(ctrlCardBlur) ctrlCardBlur.value = cardBlur;
    
    if(ctrlBmRadius) ctrlBmRadius.value = bmRadius;
    if(ctrlBmOpacity) ctrlBmOpacity.value = bmOpacity;
    if(ctrlBmBlur) ctrlBmBlur.value = bmBlur;
  }
  
  // Appearance Event Listeners
  if (ctrlCardRadius) {
      ctrlCardRadius.addEventListener('input', (e) => {
          cardRadius = e.target.value;
          updateAppearance();
          saveSetting('cardRadius', cardRadius);
      });
      ctrlCardOpacity.addEventListener('input', (e) => {
          cardOpacity = e.target.value;
          updateAppearance();
          saveSetting('cardOpacity', cardOpacity);
      });
      ctrlCardBlur.addEventListener('input', (e) => {
          cardBlur = e.target.value;
          updateAppearance();
          saveSetting('cardBlur', cardBlur);
      });
      
      ctrlBmRadius.addEventListener('input', (e) => {
          bmRadius = e.target.value;
          updateAppearance();
          saveSetting('bmRadius', bmRadius);
      });
      ctrlBmOpacity.addEventListener('input', (e) => {
          bmOpacity = e.target.value;
          updateAppearance();
          saveSetting('bmOpacity', bmOpacity);
      });
      ctrlBmBlur.addEventListener('input', (e) => {
          bmBlur = e.target.value;
          updateAppearance();
          saveSetting('bmBlur', bmBlur);
      });
  }
  
  if (resetAppearanceBtn) {
      resetAppearanceBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          // Reset JS Variables
          cardRadius = 16;
          cardOpacity = 5;
  ...
  function updateDarkMode() {
      ...
      if (isDarkMode) {
          ...
          // Optional: Update meta theme-color if needed
      } else {
          document.documentElement.setAttribute('data-theme', 'light');
          if (toggle) toggle.classList.remove('active');
      }
      
      // Re-trigger appearance update to ensure opacity variables are correct for the theme
      // updateAppearance(); // Not strictly needed if CSS variables handle it, but good practice if logic depended on it.
  }
  ...
  if (toggleHeaderBtn) {
      toggleHeaderBtn.addEventListener('click', (e) => {
          ...
          saveSetting('showHeader', showHeader);
          updateHeaderState();
      });
  }
  ...
  function createCategoryCard(node) {
        ...
        categoryCard.addEventListener('dragleave', handleDragLeave); // New: Handle leave cleanuup
  ...
  function handleDragStart(e) {
    ...
    if (li && li.dataset.id) {
        ...
        e.dataTransfer.effectAllowed = 'move';
        li.classList.add('drag-src'); // Visual style
    }
  }

  function handleDragOver(e) {
    if (e.preventDefault) e.preventDefault();
    e.dataTransfer.dropEffect = 'move'; // Action is move
    
    // Cleanup previous indicators globally (simpler/safer)
    // Note: Checking document performance, doing this on every dragover might be heavy but usually fine for simple DOM
    // Better: Scope to current categoryCard if passed? For now global is safe logic 
    document.querySelectorAll('.drop-target-top, .drop-target-bottom').forEach(node => {
        node.classList.remove('drop-target-top', 'drop-target-bottom');
    });

    const targetLi = e.target.closest('li');
    const categoryCard = e.currentTarget; // The folder card
    ...
    if (targetLi && targetLi !== dragSrcEl) {
        ...
        if (offset < rect.height / 2) {
            targetLi.classList.add('drop-target-top');
        } else {
            targetLi.classList.add('drop-target-bottom');
        }
    } else {
        // Dragging in empty space of folder
  ...
  function handleDragLeave(e) {
      // Logic to prevent flickering when moving between children
      // Often dragleave fires when entering a child element. 
      // We rely on dragover to clear specific LI classes. 
      // This is mostly to clear the CATEGORY CARD 'drag-over' class when leaving the card entirely.
      const categoryCard = e.currentTarget;
      
      // Simple check: if leaving the card, remove class
      // Note: This logic can be tricky because e.relatedTarget might be a child.
      if (!categoryCard.contains(e.relatedTarget)) {
  ...
  function handleDrop(e) {
    ...
    if (dragSrcEl) {
        dragSrcEl.classList.remove('drag-src');
    ...
    // 2. Identify Target
    const targetLi = e.target.closest('li');
    const targetCard = e.currentTarget; // Category Card
    const targetFolderId = targetCard.dataset.id;
    
    // Case A: Dropped onto specific Bookmark (Reorder)
    if (targetLi && targetLi.dataset.id && targetLi !== dragSrcEl) {
        const targetId = targetLi.dataset.id;
        
        // Calculate Position
        const rect = targetLi.getBoundingClientRect();
        const offset = e.clientY - rect.top;
        const insertAfter = offset > (rect.height / 2); // Top vs Bottom

        chrome.bookmarks.get(targetId, (results) => {
            if (chrome.runtime.lastError || !results || !results.length) return;
            const targetBm = results[0];
            
            // Define new index
            // If dragging to same folder, logic is tricky because removing src changes index. 
            // Chrome API usually handles 'move to index' logic by treating index as pre-removal state or post-removal.
            // Documentation says: "The new index of the bookmark in the parent folder."
            // If moving down: newIndex > oldIndex. If moving up: newIndex < oldIndex.
            
            let newIndex = targetBm.index;
            if (insertAfter) newIndex++;

            // Execute Move
            // destParentId is priority: target's parent (to ensure it goes into that folder)
            const destParentId = targetBm.parentId;
            
            chrome.bookmarks.move(sourceId, { parentId: destParentId, index: newIndex }, () => {
                 if (chrome.runtime.lastError) console.error(chrome.runtime.lastError);
                 loadBookmarks();
            });
        });
    } 
    // Case B: Dropped into Folder Empty Space (Append)
    else if (targetFolderId) {
        // Just move to folder, auto appends to end
        chrome.bookmarks.move(sourceId, { parentId: targetFolderId }, () => {
             if (chrome.runtime.lastError) console.error(chrome.runtime.lastError);
             loadBookmarks();
        });
    }
  ...
  if (donateBtn) {
      donateBtn.addEventListener('click', () => {
          ...
          setTimeout(() => donationModal.classList.add('visible'), 10);
      });
  }
...
}); // End DOMContentLoaded (Appended logic)
